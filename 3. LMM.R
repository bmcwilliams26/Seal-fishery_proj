...
#dive duration
(re.effects_dd <- plot_model(lme_dur_ar1, type = "est", show.values = TRUE,
                         rm.terms = c("substrateCoarse", 'substrateRocky', "sunElevationNight", "bathymetry"),
                         title = 'A) Dive Duration (s)', axis.labels = c('Trawl overlap', 'Gillnet overlap'),
                         vline.color = "#061423", dot.size = 5, line.size = 2, value.size = 5.5)+
  theme_bw()+
...
#time at bottom
(re.effects_sb <- plot_model(lme_sbtm_ar1, type = "est", show.values = TRUE,
                          rm.terms = c("substrateCoarse", 'substrateRocky', "sunElevationNight", "bathymetry"),
                          title = 'B) Time at bottom (s)', axis.labels = c('Trawl overlap', 'Gillnet overlap'),
                          vline.color = "#061423", dot.size = 5, line.size = 2, value.size = 5.5)+
    theme_bw()+
    scale_y_continuous(breaks = seq(-20, 15, by = 5))+
    theme(text = element_text(size = 14), 
          ...
          axis.text = element_text(size = 13, color = 'black')))
#complexity
(re.effects_c <- plot_model(lme_complex_ar1, type = "est", show.values = TRUE,
                          rm.terms = c("substrateCoarse", 'substrateRocky', "sunElevationNight", "bathymetry"),
                          title = 'C) Complexity (m)', axis.labels = c('Trawl overlap', 'Gillnet overlap'),
                          vline.color = "#061423", dot.size = 5, line.size = 2, value.size = 5.5)+
    theme_bw()+
    theme(text = element_text(size = 14), 
          legend.text = element_text(size=13, color = 'black'),
          axis.text = element_text(size = 13, color = 'black')))
#swim effort 
(re.effects_se <- plot_model(lme_swimeff_ar1, type = "est", show.values = TRUE,
                          rm.terms = c("substrateCoarse", 'substrateRocky', "sunElevationNight", "bathymetry"),
                          title = 'D) Swim effort', axis.labels = c('Trawl overlap', 'Gillnet overlap'),
                          digits = 4, vline.color = "#061423", axis.lim = c(-1.5, 1.5),
                          dot.size = 5, line.size = 2, value.size = 5.5) + 
    theme_bw()+
    theme(text = element_text(size = 14), 
          legend.text = element_text(size=13, color = 'black'),
          axis.text = element_text(size = 13, color = 'black'))+
...
tab_model(lme_dur_ar1,
          ...
          dv.labels= "Dive Duration")
tab_model(lme_sbtm_ar1,
          pred.labels =c("(Intercept)", "Gillnet Overlap", "Trawl Overlap", "Coarse Substrate",
                         'Rocky Substrate', 'Night', 'Bathymetry'),
          dv.labels= "Time at Bottom")
tab_model(lme_complex_ar1,
          pred.labels =c("(Intercept)", "Gillnet Overlap", "Trawl Overlap", "Coarse Substrate",
                         'Rocky Substrate', 'Night', 'Bathymetry'),
          dv.labels= "Complexity")
tab_model(lme_swimeff_ar1,
          pred.labels =c("(Intercept)", "Gillnet Overlap", "Trawl Overlap", "Coarse Substrate",
                         'Rocky Substrate', 'Night', 'Bathymetry'),
          dv.labels= "Swim Effort")
